...
class FixtureAppTest < ArchSpecTest
  ...
  def test_rails_shaped_fixture_passes
    output = StringIO.new
    status = Dir.chdir(FIXTURE_ROOT) { ArchSpec::CLI.call(["check"], output: output, error: StringIO.new) }
  ...
  def test_explain_fixture_component_assignment
    output = StringIO.new
    status = Dir.chdir(FIXTURE_ROOT) { ArchSpec::CLI.call(["explain", "app/services/create_user.rb"], output: output, error: StringIO.new) }
