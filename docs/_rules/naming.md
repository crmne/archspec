...
Naming rules check the shape and consistency of the methods a component defines. They start from `method_names`, select some of those methods by name, then assert something about them. Every check is name-based and exact, so it reports at `high` confidence.
...
`scope:` on `method_names` selects which methods a rule judges: `:instance` (the default) or `:class`. On `requires`, `scope:` instead sets the scope of the required sibling.
