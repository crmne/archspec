...
## What the engine adds to the file

A format 2 file may carry four more lists, and the Rubydex producer writes all of them. `externals` names each declaration outside the workspace that a reference, an ancestor or a receiver reached, with its kind, the gem it came from (`core` for the core library) and the methods the engine lists on both sides; `check` holds each as a constant with no file, ownable by `namespace:` or `constants:`, never a member. `ancestors` carries the engine's linearised chain per workspace namespace, instance side and class side, which the protocol rules read ahead of the parser's own chain. `aliases` carries constant and method aliases with their targets, so `cannot_reference_constants` and `cannot_call` match a name under its aliases. `diagnostics` carries the engine's own findings by rule and line; the census counts them and a `DynamicAncestor` on a class doubts the findings in it. A `definitions` entry may state `arity`, `keywords` and `block`, and a `calls` entry its `scope`, so `must_implement arity:`, `cannot_take` and `cannot_call receiver:` read them. Misses gained their causes: `ancestors_dynamic`, `alias_cycle`, `external_defined_in_workspace`.
