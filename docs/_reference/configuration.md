...
`since:` dates the rule. ArchSpec asks git when each offending line last changed: a line older than the date is listed under its own heading and does not fail the check, a newer one fails as usual. Without git, or for a line not yet committed, the finding counts as undated and fails; the run says so once. The date is read from `git blame`, one call per file that holds a finding of a dated rule, so a line that moved carries the date it moved and a shallow clone carries the date of its oldest commit. Only dated rules ask, and only their findings are looked up: on Discourse, ten findings in eight files cost about three tenths of a second on top of a three-second check.
