...
```ruby
models.cannot_use :controllers,
  because: "a model that knows the request cannot be used off it",
  since: "2024-01-01"
```

Every rule-creating call takes `because:` and `since:`. The reason prints beside each finding and travels in the JSON output; the presets carry one for each of their rules. Neither keyword enters a finding's todo id, so adding them to a rule you already run moves no todo entry. Declaring the same rule twice merges them; two different reasons or two different dates for one rule are an error when the file loads.

`since:` dates the rule. ArchSpec asks git when each offending line last changed: a line older than the date is listed under its own heading and does not fail the check; a newer one, or one committed on the date itself, fails as usual. Without git, or for a line not yet committed, the finding counts as undated and fails; the run says so once. The date is read from `git blame`, one call per file that holds a finding of a dated rule, so a line that moved carries the date it moved; a line older than a shallow clone's history cannot be dated and counts as undated. The JSON output carries the rule's `since` date and the finding's `age` (`before`, `after` or `unknown`). Only dated rules ask, and only their findings are looked up: on Discourse, ten findings in eight files cost about three tenths of a second on top of a three-second check.
